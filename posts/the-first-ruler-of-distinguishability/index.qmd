...
There is an important computational caveat. For a model with $N$ parameters, the FIM is an $N \times N$ matrix. In modern neural networks, where $N$ can be millions or billions, explicitly constructing this matrix is impossible. Practical methods therefore rely on matrix-free Fisher-vector products, diagonal approximations, low-rank approximations, or structured block approximations such as K-FAC. These approximations change the computation, but not the underlying geometric object.
...
\sum_{i,j}
F^{(\theta)}_{ij}
\frac{\partial \theta_i}{\partial \alpha^a}
\frac{\partial \theta_j}{\partial \alpha^b}.
