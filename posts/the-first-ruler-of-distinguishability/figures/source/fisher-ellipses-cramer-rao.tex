...
\tikzset{
  ...
  mbox/.style={fill=none, draw=deepblue!32,
               line width=0.8pt, rounded corners=5pt, inner sep=8pt},
  pbox/.style={fill=none, draw=softgreen!38,
               line width=0.8pt, rounded corners=5pt, inner sep=8pt},
}
\begin{document}
\begin{tikzpicture}[font=\small, scale=0.90]
  \draw[deepblue!18, rounded corners=5pt, thick]
    (-5.3,-3.1) rectangle (5.3,2.8);
  \foreach \ya in {-1.7,-0.2,1.3}{
    \draw[deepblue!12,thick]
  ...
  \node[deepblue,above left=3pt,font=\footnotesize\bfseries]
    at (ptA) {$\theta_A$};
  \draw[deepblue,thick,fill=deepblue,fill opacity=0.08,
  ...
  \draw[warmgold!90!black,thick,rounded corners=3pt,
        fill=warmgold,fill opacity=0.06]
  ...
  \node[font=\scriptsize,align=center,text width=3.0cm,gray!58]
    at (-3.40,-2.72)
